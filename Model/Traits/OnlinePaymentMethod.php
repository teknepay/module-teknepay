...
namespace Teknepay\Teknepay\Model\Traits;

/**
 * Trait for defining common variables and methods for all Payment Solutions
 * Trait OnlinePaymentMethod
 * @package Teknepay\Teknepay\Model\Traits
 */
trait OnlinePaymentMethod
{
    /**
     * @var \Teknepay\Teknepay\Model\Config
     */
    protected $_configHelper;
    /**
     * @var \Teknepay\Teknepay\Helper\Data
     */
    ...
    protected function processReferenceTransaction(
        ...
        $teknepay = "\\Teknepay\\$transactionType";
        $teknepay = new $teknepay;

        $teknepay->setParentUid($data['reference_id']);
        $teknepay->money->setAmount($data['amount']);
        $teknepay->money->setCurrency($data['currency']);

        if (strtolower($transactionType) == $helper::REFUND)
          $teknepay->setReason($data['reason']);

        $responseObject = $teknepay->submit();
